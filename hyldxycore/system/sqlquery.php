...
class SQLQuery {
     ...
     */
    public function sqlUpdate(string $scoresaberID, string $mapHash, array $data, string $table): void {
        foreach ($data as $key => $value) {
            $keys[]     = $key;
            $values[]   = $value;
        }
        ...
        $sql .= "WHERE scoresaberID = :scoresaberID AND mapID = :mapHash";
        ...
        $sqlUpdate->bindValue(":scoresaberID", $scoresaberID, PDO::PARAM_STR);
        $sqlUpdate->bindValue(":mapHash", $mapHash, PDO::PARAM_STR);
